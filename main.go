...
func InterpolateLagrangePolynomial(x float32, steps int, Values []point) float32{
	...
	for i := 0; i < steps; i++ {
		...
		for j := 0; j < steps; j++ {
			...
			basics_pol *= (x - Values[j].x)/(Values[i].x - Values[j].x)
		}
		lagrange_pol += basics_pol*Values[i].y
	}
	return lagrange_pol
}
